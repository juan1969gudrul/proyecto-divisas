...
const generateRandomMovement = (date) => {
  ...
  const amount = isDeposit 
    ? faker.number.float({ min: 100, max: 5000, precision: 0.01 })  // Depósitos entre 100€ y 5000€
    : -faker.number.float({ min: 50, max: 2000, precision: 0.01 }); // Retiros entre 50€ y 2000€
...
const generateRandomAccount = () => {
  ...
  return {
    owner,
    movements,
    interestRate: faker.number.float({ min: 0.5, max: 2.5, precision: 0.1 }), // Tasa de interés entre 0.5% y 2.5%
